...
describe('tile planning', () => {
    test('returns a stable sorted warm tile list', () => {
        ...
        expect([...warmupPlan.tileKeys]).toEqual(
            [...warmupPlan.tileKeys].sort()
        )
    })
    ...
    test('builds a bounded local terrain window around the track', () => {
        ...
        expect(bounds.minY).toBeLessThanOrEqual(
            preparedTrack.mercatorBounds.minY
        )
        expect(bounds.maxY).toBeGreaterThanOrEqual(

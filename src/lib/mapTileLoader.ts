 ...
 */
export function fetchImageryTexture(z: number, x: number, y: number): Promise<Texture> {
    const key = `${z}/${x}/${y}`;
    ...
    const promise = new Promise<Texture>((resolve, reject) => {
        ...
        console.warn(`[TileLoader] Failed to load imagery for ${key}, using placeholder:`, err);
 ...
 */
export function fetchElevationTile(z: number, x: number, y: number): Promise<Float32Array> {
    const key = `${z}/${x}/${y}`;
    ...
    const promise = (async () => {
        ...
        try {
            ...
            console.warn(`[TileLoader] Failed to load/decode elevation for ${key}, using flat:`, err);

...
function getSegmentIndexAtDistance(distanceM: number, cumulativeDistancesM: number[]) {
...
export function buildSegmentedTrackGeometry({
  ...
  normals.push(new Vector3().crossVectors(firstReference, tangents[0]).normalize());
  binormals.push(new Vector3().crossVectors(tangents[0], normals[0]).normalize());

  for (let i = 1; i < pathPoints.length; i++) {
    const projectedNormal = normals[i - 1]
      .clone()
      .sub(tangents[i].clone().multiplyScalar(normals[i - 1].dot(tangents[i])));

    if (projectedNormal.lengthSq() < 1e-6) {
      const fallbackReference = Math.abs(tangents[i].dot(up)) > 0.98 ? altUp : up;
      projectedNormal.copy(new Vector3().crossVectors(fallbackReference, tangents[i]));
    }

    normals.push(projectedNormal.normalize());
    binormals.push(new Vector3().crossVectors(tangents[i], projectedNormal).normalize());
  }
  ...
  for (let segmentIndex = 0; segmentIndex < sampleSegments; segmentIndex++) {
    ...
    const distanceRatio = sampleSegments > 0 ? (segmentIndex + 0.5) / sampleSegments : 0;
    const distanceAtCenter = totalDistanceM * distanceRatio;
    const speedIndex = getSegmentIndexAtDistance(distanceAtCenter, cumulativeDistancesM);
    const speed = segmentSpeeds[speedIndex] ?? 0;
    const color = useSpeedColors ? getSpeedColor(speed, maxSpeedKmh) : getNeutralTrackColor();
    ...
    for (let radialIndex = 0; radialIndex < radialSegments; radialIndex++) {
      ...
      const startOffsetA = startNormal
        ...
        .add(startBinormal.clone().multiplyScalar(Math.sin(angle) * radius));
      const startOffsetB = startNormal
        .clone()
        .multiplyScalar(Math.cos(nextAngle) * radius)
        .add(startBinormal.clone().multiplyScalar(Math.sin(nextAngle) * radius));
      const endOffsetA = endNormal
        .clone()
        .multiplyScalar(Math.cos(angle) * radius)
        .add(endBinormal.clone().multiplyScalar(Math.sin(angle) * radius));
      const endOffsetB = endNormal
        .clone()
        .multiplyScalar(Math.cos(nextAngle) * radius)
        .add(endBinormal.clone().multiplyScalar(Math.sin(nextAngle) * radius));
  ...
  geometry.setAttribute("normal", new Float32BufferAttribute(normalsAttribute, 3));

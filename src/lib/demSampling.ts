...
function latLonToTileCoordinate(lat: number, lon: number, zoom: number): TileCoordinate {
...
async function fetchTerrariumTile(zoom: number, x: number, y: number): Promise<TileSample> {
...
export async function sampleTrackElevations(
  ...
  for (let start = 0; start < preparedTrack.points.length; start += SAMPLE_BATCH_SIZE) {
    const batch = preparedTrack.points.slice(start, start + SAMPLE_BATCH_SIZE);
    const updates: Array<{ index: number; height: number }> = [];

    for (let offset = 0; offset < batch.length; offset++) {
      const point = batch[offset];
      const tile = latLonToTileCoordinate(point.lat, point.lon, TERRARIUM_ZOOM);
      const tileSample = await fetchTerrariumTile(TERRARIUM_ZOOM, tile.x, tile.y);
      updates.push({
        index: start + offset,

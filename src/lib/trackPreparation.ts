...
function findNearestPreparedIndex(
    points: PreparedTrackPoint[],
...
export function prepareTrackData(gpxContent: string): PreparedTrackData {
    ...
    for (let i = 0; i < gpxData.points.length; i++) {
        ...
        const shouldInclude =
            ...
            Math.hypot(
                mercator.x - previousMercatorX,
                mercator.y - previousMercatorY
        ...
        if (preparedPoints.length > 0) {
            ...
            segmentSpeeds.push(
                averageSegmentSpeed(
                    stats.pointSpeeds,
                    previousPrepared.originalIndex,
                    i
                )
            )
        }
    ...
    return {
        ...
        highestPreparedIndex:
            findNearestPreparedIndex(preparedPoints, stats.highestPointIndex) ??
            0,
        fastestPreparedIndex: findNearestPreparedIndex(
            preparedPoints,
            stats.fastestPointIndex
        ),
    }
}

...
export function computeGpxStats(name: string, points: GpxPoint[]): GpxStats {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (i > 0) {
      ...
      // Distance
      const segDistM = haversineMeters(prev.lat, prev.lon, curr.lat, curr.lon);
      ...
      if (prev.time && curr.time) {
        hasTime = true;
        const dtS = (new Date(curr.time).getTime() - new Date(prev.time).getTime()) / 1000;
        if (dtS > 0) {
          totalTimeS += dtS;
  ...
  const avgSpeedKmh = hasTime && totalTimeS > 0 ? (totalDistanceM / totalTimeS) * 3.6 : null;

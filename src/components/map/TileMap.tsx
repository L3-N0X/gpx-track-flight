...
import { fetchImageryTexture, fetchElevationTile, clearTileCache } from '../../lib/mapTileLoader'
...
function sampleHeightFromGrid(grid: Float32Array, u: number, v: number): number {
    const px = u * 255;
    const py = v * 255;
...
function displaceGeometry(geometry: PlaneGeometry, elevationGrid: Float32Array, z: number, width: number) {
...
class TileCache {
    ...
    public async load(key: string, z: number, x: number, y: number, triggerUpdate: () => void): Promise<void> {
...
function MapTileMesh({ z, x, y, colorTexture, elevationGrid }: MapTileMeshProps) {
    ...
    return (
        <mesh position={[centerX, 0, centerZ]} geometry={geometry} receiveShadow castShadow>
...
export function TileMap({ preparedTrack, onWarmupChange, worldOrigin }: TileMapProps) {
    ...
    const trackElevation = useMemo(() => {
        ...
        const sum = preparedTrack.points.reduce((acc, p) => acc + p.smoothedElevationM, 0);
    ...
    const rootsLoaded = useMemo(() => {
        ...
        return rootTiles.every(t => tileCache.isLoaded(`${t.z}/${t.x}/${t.y}`));
    }, [rootTiles, tileCache]);
    ...
    useFrame((state) => {
        ...
        const activeKeys = new Map<string, { key: string; z: number; x: number; y: number }>();
        ...
        const isTileVisible = (z: number, x: number, y: number): boolean => {
            ...
            const globalBox = new Box3(
                new Vector3(bounds.minX - worldOrigin.x - frustumMargin, -2000, bounds.minZ + worldOrigin.y - frustumMargin),
                new Vector3(bounds.maxX - worldOrigin.x + frustumMargin, 10000, bounds.maxZ + worldOrigin.y + frustumMargin)
            );
        ...
        const evaluateTile = (z: number, x: number, y: number) => {
            ...
            if (shouldSubdivide(z, x, y)) {
                ...
                const allChildrenLoaded = childKeys.every(k => tileCache.isLoaded(k));

...
export function Track({ preparedTrack, onReadyChange, onSamplingStatusChange }: TrackProps) {
  ...
  }, [curveRef, onSamplingStatusChange, preparedTrack, progressRef, setSnappedPoints]);
  ...
  const curve = useMemo(
    () => (smoothedPath.length >= 2 ? new CatmullRomCurve3(smoothedPath) : null),
    [smoothedPath],
  );
      ...
      {fastestPoint && preparedTrack.stats.maxSpeedKmh !== null && (
        <group position={[fastestPoint.x, fastestPoint.y + 100, fastestPoint.z]}>
          ...
          <Html center className="pointer-events-none">
            <div className="flex flex-col items-center drop-shadow-md">
              <div className="bg-slate-900/80 font-bold text-white px-2 py-1 rounded border border-white/20 whitespace-nowrap mb-1">
                🚀 {preparedTrack.stats.maxSpeedKmh.toFixed(1)} km/h
              </div>

  ...
  return (
    <div className="absolute top-4 left-4 z-20 w-80 bg-slate-950/85 text-slate-100 border border-slate-700 rounded-lg shadow-xl backdrop-blur-md pointer-events-none">
      <div className="px-3 py-2 border-b border-slate-800 flex items-center justify-between">
        <span className="text-sm font-semibold tracking-wide">Debug Overlay</span>
      ...
      <div className="p-3 space-y-3 text-xs">
        <section className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            <span>FPS</span>
            <span>{metrics ? formatNumber(metrics.fps) : "..."}</span>
            <span>Frame ms</span>
            <span>{metrics ? formatNumber(metrics.frameTimeMs) : "..."}</span>
        ...
        <section className="space-y-1">
          <div className="font-semibold text-slate-300">Map State</div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            ...
            <span>
              {metrics?.visibleMapMeshes ?? 0}/{metrics?.totalMapMeshes ?? 0}
            </span>
        ...
        <section className="space-y-1">
          <div className="font-semibold text-slate-300">Track DEM</div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            <span>Sampled</span>
            <span>
            ...
            <span>Error</span>
            <span className="truncate">{samplingStatus?.error ?? "-"}</span>
          </div>
        </section>
        ...
        <section className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            ...
            <span>Speed</span>
            <span>{metrics ? `${formatNumber(metrics.speed)}x` : "..."}</span>
            <span>Progress</span>
            <span>{metrics ? `${(metrics.flightProgress * 100).toFixed(1)}%` : "..."}</span>
        ...
        <section className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            <span>X</span>
            <span>{metrics ? formatNumber(metrics.cameraX) : "..."}</span>
            <span>Y</span>
            <span>{metrics ? formatNumber(metrics.cameraY) : "..."}</span>
            <span>Z</span>
            <span>{metrics ? formatNumber(metrics.cameraZ) : "..."}</span>
          </div>
        </section>
      </div>
    </div>
